...
int main()
{	
	CLThreadProxy *cal_thread[CALCULATE_THREAD_NUM];  //事先分配一个集合存储多个线程
	...
	control = new CLThreadProxy(new CLControl, "control" ,true);
	control->Run(0);
	
	user = new CLNonThreadProxy(new CLUser, "user");
	user->Run(0);
	return 0;
}
