...
class CLMyMsgProcessor : public CLMsgLoopManagerForMsgQueue
{
public:
	CLMyMsgProcessor(CLMessageQueue *pQ) : CLMsgLoopManagerForMsgQueue(pQ)
	{
	}
	...
	virtual CLStatus MessageDispatch(CLMessage *pM)
	{
...
int main()
{
	CLMessageQueue *pQ = new CLMessageQueue();
	...
	CLAddMessage *paddmsg3 = new CLAddMessage(15, 62);
	pQ->PushMessage(paddmsg3);

	cout << "start" << pQ->start <<endl;
	cout << "tail" << pQ->tail <<endl;
