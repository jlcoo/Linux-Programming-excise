...
CLThreadProxy::CLThreadProxy(CLMessageObserver *pMsgObserver, const char *pstrThreadName,  bool bWaitForDeath, CLMessageQueue *m_pMsgQueue)
{
	m_bWaitForDeath = bWaitForDeath;
	
	m_pThread = new CLThread(new CLExecutiveFunctionForMsgLoop(new CLMsgLoopManagerForMsgQueue(pMsgObserver, pstrThreadName, m_pMsgQueue)), bWaitForDeath);
}
